...
cluster <- kmeans(data_fp_gua, centers = 5)
...
ggplot(data_fp_gua, aes(x = edad_per, y = fall_les, color = as.factor(cluster$cluster))) +
  ...
  geom_point(data = as.data.frame(cluster$centers),
             ...
             color = "black", size = 4, shape = 17) +
  labs(title = "Vic Edad vs fall_les") +
  theme_minimal()
```
...
ggplot(data_fp_gua, aes(x = edad_per, y = tipo_veh, color = as.factor(cluster$cluster))) +
  ...
  geom_point(data = as.data.frame(cluster$centers),
             ...
             color = "black", size = 4, shape = 17) +
  labs(title = "Vic Edad vs tipo vehiculo") +
  theme_minimal()
```
```{r}
cluster <- kmeans(data_fp_gua, centers = 5)
...
```{r}
set.seed(123)
kmeans_final <- kmeans(data_fp_scaled, centers = 5, nstart = 25)
```
```{r}
...
ggplot(data_fp_gua_cluster, aes(x = edad_per, y = tipo_eve, color = as.factor(cluster))) +
  geom_jitter(alpha = 0.6, width = 0.2, height = 0.2) +
  labs(
    title = "Clusters de accidentes en Guatemala (2015–2023)",
    subtitle = "Relación entre edad y tipo de evento",
    x = "Edad de la persona involucrada",
    y = "Tipo de evento",
    color = "Cluster"
  ) +
  theme_minimal() +
...
# --- 1. Dataset con clusters ya asignados ---
# Asegúrate de tener esto antes:
# data_fp_gua_cluster (datos originales + columna cluster)
# kmeans_final (modelo K-means)
# data_fp_scaled (versión numérica estandarizada)

# Convertimos las variables a numéricas para el análisis
data_num <- data_fp_gua %>%
  mutate(across(everything(), ~ as.numeric(as.factor(.x)))) %>%
  mutate(cluster = kmeans_final$cluster)

# --- 2. Generar todas las combinaciones posibles de pares de variables ---
vars <- names(data_num)[!names(data_num) %in% "cluster"]
pairs <- combn(vars, 2, simplify = FALSE)

# --- 3. Calcular distancia promedio entre centroides por cada par ---
...
# --- 4. Ordenar las combinaciones según la mejor separación ---
ranked_pairs <- tibble(
  var1 = sapply(pairs, `[`, 1),
  var2 = sapply(pairs, `[`, 2),
...
```
```{r}
best_pair <- ranked_pairs[1, c("var1", "var2")]

v1 <- best_pair$var1
v2 <- best_pair$var2

# Asegurar que cluster sea un factor
data_fp_gua_cluster$cluster <- as.factor(data_fp_gua_cluster$cluster)

ggplot(data_fp_gua_cluster, aes_string(x = v1, y = v2, color = "cluster")) +
  geom_jitter(alpha = 0.6, width = 0.2, height = 0.2) +
  labs(
    title = paste("Mejor visualización de clusters según separación de centroides"),
    subtitle = paste("Ejes:", v1, "vs", v2),
    x = v1,
    y = v2,
    color = "Cluster"
  ) +
